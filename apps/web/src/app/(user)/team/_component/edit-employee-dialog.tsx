...
const formSchema = z.object({
  ...
  status: z.nativeEnum(IEmployeeStatus, {
...
export function EditEmployeeDialog({
  ...
  const onSubmit = async (data: IEditEmployeeFormValues) => {
    ...
    try {
      ...
      const { employee: newEmployee, error } = await updateEmployee(
        employee.id,
        updatedEmployee,
      );
      if (newEmployee) {
        onUpdateEmployee(employee.id, newEmployee);
        form.reset();
        onOpenChange(false);
      } else {
        setError(error);
