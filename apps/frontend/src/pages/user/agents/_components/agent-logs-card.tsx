...
export const AgentLogsCard: React.FC<AgentLogsCardProps> = ({
    logs,
    loading = false,
    entityId,
    totalCount,
    ...
    const {
        ...
        fetchPage: async ({ page, limit, offset }) => {
            ...
            const res = await axiosInstance.get(`/activity-log/entity/agent/${entityId}`, {
    ...
    const getActionMeta = (log: IAgentLog) => {
        const action = log.action.toLowerCase();

        if (action.includes("payout") || action.includes("paid")) {
            return {
                label: "Payout Event",
                icon: Banknote,
        ...
        if (action.includes("commission") || action.includes("rate")) {
            return {
    ...
    const renderLogItem = (log: IAgentLog) => {
        ...
        return (
            <div key={log.id} className="relative flex items-start gap-3 group">
                ...
                <div className="flex-1 min-w-0 bg-muted/30 hover:bg-muted/50 transition-colors p-3 rounded-lg border border-border/60 space-y-2">
                    ...
                    {log.metadata?.bookingNumber && (
                        <div className="flex items-center gap-2 text-[11px] text-muted-foreground font-mono">
                            <span>Booking: #{log.metadata.bookingNumber}</span>
                            {log.metadata?.newStatus && (
                                <span>• Status: {log.metadata.newStatus}</span>
                            )}
                        </div>
                    )}
                </div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    History of payouts, commission updates, and profile edits.
                                </p>
                            </div>
                        </div>
                ...
                <DialogContent className="max-w-2xl sm:max-w-2xl w-[95vw] h-[80vh] max-h-[85vh] flex flex-col p-0 gap-0 overflow-hidden">
                    <DialogHeader className="p-4 sm:p-5 border-b shrink-0">
                        ...
                        <DialogDescription className="text-xs text-muted-foreground mt-0.5">
                            Complete timeline of payouts, commission adjustments, and agent actions.
                        </DialogDescription>
                    </DialogHeader>
