...
interface CustomerLogsCardProps {
    logs: ICustomerLog[];
    loading?: boolean;
    entityId?: string;
    totalCount?: number;
...
export const CustomerLogsCard: React.FC<CustomerLogsCardProps> = ({
    logs,
    loading = false,
    entityId,
    totalCount,
    ...
    const {
        ...
        fetchPage: async ({ page, limit, offset }) => {
            ...
            const res = await axiosInstance.get(`/activity-log/entity/customer/${entityId}`, {
    ...
    const getActionMeta = (log: ICustomerLog) => {
        const action = log.action.toLowerCase();

        if (action.includes("blacklisted")) {
            return {
        ...
        if (action.includes("whitelist") || action.includes("unblacklisted")) {
            return {
                label: "Blacklist Removed",
                icon: ShieldCheck,
    ...
    const renderLogItem = (log: ICustomerLog) => {
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    History of customer profile updates and transactions.
                                </p>
                            </div>
                        </div>
                ...
                <DialogContent className="max-w-2xl sm:max-w-2xl w-[95vw] h-[80vh] max-h-[85vh] flex flex-col p-0 gap-0 overflow-hidden">
                    <DialogHeader className="p-4 sm:p-5 border-b shrink-0">
                        ...
                        <DialogDescription className="text-xs text-muted-foreground mt-0.5">
                            Complete timeline of customer profile updates, bookings, and payments.
                        </DialogDescription>
                    </DialogHeader>
...
export default CustomerLogsCard;
