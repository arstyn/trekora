...
export function CustomTemplateBuilder() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const handleColumnChange = (columnId: string, field: keyof CustomColumn, value: any) => {
    setFormData(prev => ({
      ...prev,
  ...
  const saveTemplate = async () => {
    if (!formData.name || !formData.entityType || !formData.columns?.length) {
    ...
    try {
      ...
      if (isEditing && selectedTemplate?.id) {
        await AxiosRequest.put(`/import/templates/${selectedTemplate.id}`, payload);
  ...
  const downloadTemplate = async (templateId: string) => {
    try {
      const response = await AxiosRequest.get(`/import/templates/${templateId}/download`, {
        responseType: 'blob'
      });
  ...
  const editTemplate = (template: CustomTemplate) => {
    ...
    setSelectedEntityType(template.entityType);
    setIsEditing(true);
    setIsCreating(false);
  };
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    {formData.columns?.map((column) => (
                      <Card key={column.id} className="border-l-4 border-l-blue-500">
                        <CardContent className="pt-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <div className="space-y-2">
                              <Label>Excel Column Name *</Label>
                              <Input
                                value={column.excelColumnName}
                                onChange={(e) => handleColumnChange(column.id, 'excelColumnName', e.target.value)}
                            ...
                            <div className="space-y-2">
                              ...
                              <Select 
                                ...
                                onValueChange={(value) => handleColumnChange(column.id, 'entityField', value)}
                              >
                                <SelectTrigger>
                            ...
                            <div className="space-y-2">
                              ...
                              <Select 
                                ...
                                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                                onValueChange={(value: any) => handleColumnChange(column.id, 'dataType', value)}
                              >
                                <SelectTrigger>
                                  <SelectValue />
                                </SelectTrigger>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex items-center space-x-2">
                                <Checkbox
                                  ...
                                  onCheckedChange={(checked) => handleColumnChange(column.id, 'isRequired', checked)}
                                />
                                <Label htmlFor={`required-${column.id}`}>Required field</Label>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="space-y-2 mt-4">
                            ...
                            <Input
                              ...
                              onChange={(e) => handleColumnChange(column.id, 'validation', e.target.value)}
                          ...
                          <div className="flex items-center justify-between mt-4">
                            <div className="flex items-center space-x-2">
                              <Switch
                                ...
                                onCheckedChange={(checked) => handleColumnChange(column.id, 'isVisible', checked)}
                              />
                              <Label htmlFor={`visible-${column.id}`}>Include in template</Label>
                            </div>
              ...
              {templates.length === 0 ? (
                ...
                <div className="space-y-4">
                  {templates.map((template) => {
                    const Icon = getEntityIcon(template.entityType);
                    return (
