...
export const uploadFiles = async (
    ...
    try {
        const response = await axiosInstance.post<FileUploadResponse[]>(
            `${import.meta.env.VITE_API_BASE_URL?.replace("/api", "")}/file-manager/upload`,
            formData,
            {
                headers: {
...
export const deleteFile = async (fileId: string): Promise<void> => {
    try {
        await axiosInstance.delete(
            `${import.meta.env.VITE_API_BASE_URL?.replace("/api", "")}/file-manager/${fileId}`,
        );
    } catch (error) {
...
export const getFilesByEntity = async (
    ...
    try {
        ...
        const response = await axiosInstance.get<FileUploadResponse[]>(
            `${import.meta.env.VITE_API_BASE_URL?.replace("/api", "")}/file-manager/related-id/${relatedId}`,
        );
        return response.data;
    } catch (error) {
