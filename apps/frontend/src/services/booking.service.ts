...
export class BookingService {
  ...
  // Update booking
  static async updateBooking(id: string, updateData: IUpdateBookingRequest): Promise<IBooking> {
  ...
  static async addPayment(bookingId: string, paymentData: Omit<IBookingPayment, 'id' | 'status'>): Promise<IBooking> {
    const response = await axiosInstance.post(`${this.baseUrl}/${bookingId}/payments`, paymentData);
    return response.data;
  }
  ...
  static async getPackages(): Promise<Array<{ id: string; name: string; price: number }>> {
    const response = await axiosInstance.get('/packages');
    return response.data;
  }

  // Get customers (for dropdown/selection)
  static async getCustomers(): Promise<Array<{ id: string; name: string; email: string; phone: string }>> {
    const response = await axiosInstance.get('/customers');
    return response.data;
  }

  // Search customers by name or email
  static async searchCustomers(query: string): Promise<Array<{ id: string; name: string; email: string; phone: string }>> {
  ...
  static getPaymentStatus(advancePaid: number, totalAmount: number): 'none' | 'partial' | 'full' {
  ...
  static validateBookingData(data: ICreateBookingRequest): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    if (data.numberOfPassengers < 1) errors.push('At least one passenger is required');
    ...
    if (!data.passengers || data.passengers.length === 0) {
      ...
      data.passengers.forEach((passenger, index) => {
        if (!passenger.fullName) errors.push(`Passenger ${index + 1}: Name is required`);
        if (!passenger.age || passenger.age < 0) errors.push(`Passenger ${index + 1}: Valid age is required`);
        if (!passenger.emergencyContact) errors.push(`Passenger ${index + 1}: Emergency contact is required`);
      });
    }
